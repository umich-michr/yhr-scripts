...
def test_enrich_dataframe():
    ...
    df = pd.DataFrame({
        'SOURCE_ADDRESS': ['192.168.1.1', '8.8.8.8']
    })

    # Create sample geolocation data
    geolocation_data = {
        '192.168.1.1': {'city': 'New York', 'region': 'NY', 'country': 'US', 'postal': '10001', 'org': 'Example Org'},
        '8.8.8.8': {'city': 'Mountain View', 'region': 'CA', 'country': 'US', 'postal': '94043', 'org': 'Google'}
    }
...
def test_enrich_dataframe_missing_ip():
    ...
    expected_values = {
        '192.168.1.1': {'city': 'New York', 'region': 'NY', 'country': 'US', 'postal': '10001', 'org': 'Example Org'},
        '8.8.8.8': {'city': 'unknown', 'region': 'unknown', 'country': 'unknown', 'postal': 'unknown', 'org': 'unknown'}
    }

    for field in ['city', 'region', 'country', 'postal', 'org']:
        assert enriched_df[field].tolist() == [expected_values[ip][field] for ip in df['SOURCE_ADDRESS']]
